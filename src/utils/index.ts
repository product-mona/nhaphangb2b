...
class Format {
	...
	formatAfterUploadImage = async (defaultImage?: string, uploadedImage?: any) => {
	...
	addUrlForImage = (imageName?: string) => (!imageName ? imageName : process.env.NEXT_PUBLIC_IMAGE_URL + imageName)
	...
	toVietnamese(number: number) {
		...
		return `${rsString.replace(/[0-9]/g, '').replace(/ /g, ' ').replace(/ $/, '')} việt nam đồng.`
	}
