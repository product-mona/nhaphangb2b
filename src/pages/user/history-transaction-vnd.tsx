	...
	return (
		<React.Fragment>
			...
			<div className="flex space-x-4 mb-4">
				<div className="cardTopTable rounded-none items-center !py-2 tableBox">
					...
					<div className="text-right sm:ml-4">
						...
						<span className="text-[#f14f04] font-bold text-base">{_format.getVND(data?.Items?.[0]?.TotalAmount4 || 0)}</span>
					</div>
				</div>
				...
				<div className="cardTopTable rounded-none items-center !py-2 tableBox">
					...
					<div className="text-right sm:ml-4">
						<p>Số dư hiện tại</p>
						<span className="text-blue font-bold text-base">{_format.getVND(data?.Items?.[0]?.Wallet || 0)}</span>
					</div>
				</div>
			</div>
			<div className="tableBox">
				...
				<div className="mt-4">
					<HistoryTransactionVNDTable data={data?.Items} filter={filter} handleFilter={handleFilter} loading={isFetching} />
				</div>
			</div>
		</React.Fragment>
	)
