...
const Index: TNextPageWithLayout = () => {
	...
	const _onPress = async (data: TConfig) => {
		// console.log(data.thumbnailList) // array
		// console.log(data)
		const fmData = {
			...data,
			ImageBanner1: data.thumbnailList[0]?.url || '',
	...
	const { isFetching } = useQuery(['articleList', 1], () => configuration.getByID(1), {
		onSuccess: (data) => {
			const fmThumbnails = []
			if (!!data.Data.ImageBanner1) {
				fmThumbnails[0] = {
					uid: `-1`,
					name: data.Data.ImageBanner1,
					status: 'done',
					url: data.Data.ImageBanner1
				}
			}
			if (!!data.Data.ImageBanner2) {
				fmThumbnails[1] = {
					uid: `-2`,
					name: data.Data.ImageBanner2,
					status: 'done',
					url: data.Data.ImageBanner2
				}
			}
			if (!!data.Data.ImageBanner3) {
				fmThumbnails[3] = {
					...
					status: 'done',
					url: data.Data.ImageBanner3
				}
			}
			reset({
				...data.Data,
				thumbnailList: fmThumbnails
			})
			setConfigData({
				...data.Data,
				thumbnailList: fmThumbnails
				...
				>
					<TabPane tab="Cấu hình chung" key="1">
						<div id="general" className={clsx(className, 'p-0 pt-2')}>
							<ConfigurationGeneral control={control} data={configData} />
						</div>
					</TabPane>
