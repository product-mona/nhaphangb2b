		...
		() => reportHistoryPayWallet.getList(filter).then((res) => res.Data),
		{
			onSuccess: (data) => {
				setChartData({
					...
					TotalRecivePaymentTransport: data?.Items[0]?.TotalRecivePaymentTransport
				})
			},
			onError: () => {
				showToast({
	...
	return (
		<div className="tableBox">
			...
			<div className="text-lg text-[#333]">
				Tổng số tiền giao dịch: <span className="font-bold">{_format.getVND(data?.Items[0]?.TotalAmount || 0)}</span>
			</div>
