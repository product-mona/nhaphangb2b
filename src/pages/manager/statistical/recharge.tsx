...
const Index: TNextPageWithLayout = () => {
	...
	const handleFilter = (username: string, bankId: number, fromDate: string, toDate: string) => {
		setUsername(username)
		setBankId(bankId)
		setFromDate(fromDate)
		setToDate(toDate)
	}
	const [rechargePagination, setRechargePagination] = useState<TablePaginationConfig>(defaultPagination)
	const [withdrawPagination, setWithdrawPagination] = useState<TablePaginationConfig>(defaultPagination)

	const { data: usertRechargeReportData, isFetching: isFetchingRecharge } = useQuery(
		[
	...
	const { data: userWithDrawReportData, isFetching: isFetchingWithdraw } = useQuery(
		[
		...
		<div className="">
			<div className={`${styleBorder} flex`}>
				...
				<div className="w-[60rem] m-auto px-[30px]">
					<StatisticalRechargeChart dataChart={{ totalRecharge, totalWithdraw }} />
				</div>
			</div>
			<div className={`${styleBorder}`}>
				...
				<div className="mt-4 text-right">
					<Pagination
						total={rechargePagination?.total || 0}
						current={rechargePagination?.current}
						pageSize={rechargePagination?.pageSize}
						onChange={(page, pageSize) => {
							setRechargePagination({ ...rechargePagination, current: page, pageSize: pageSize })
						}}
					/>
				</div>
			</div>
			...
			<div className={`${styleBorder}`}>
				...
				<div className="mt-4 text-right">
					<Pagination
						...
						pageSize={withdrawPagination?.pageSize}
						onChange={(page, pageSize) => {
							setWithdrawPagination({ ...withdrawPagination, current: page, pageSize: pageSize })
						}}
					/>
				</div>
			</div>
