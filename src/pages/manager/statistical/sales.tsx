...
const Index: TNextPageWithLayout = () => {
	...
	const handleExportExcelOrder = async (storeIndex: 0 | 1, isCNY: boolean) => {
		try {
		...
		<div className="">
			<div className={`${styleBorder}`}>
				<SalesFilter
					...
					type={type}
					handleType={() => setType((type) => (type === 'detail' ? 'sum' : 'detail'))}
					resetPagination={resetPagination}
				/>
				<SalesMoneyStatisticChart type={type} dataChart={totalOverviewData?.Data} />
			</div>
