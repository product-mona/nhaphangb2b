...
import { Layout, NotFound, PersonalRechargeFilter, showToast, toast, WithDrawalHistoryForm, WithDrawalHistoryTable } from '~/components'
...
const boxTop = 'col-span-1 tableBox cardTopTable py-4 px-3 rounded-none items-center'
...
const Index: TNextPageWithLayout = () => {
	...
	const {
		...
		isFetching,
		isError
	} = useQuery(['clientWithdrawData', { ...filter }], () => withdraw.getList(filter).then((res) => res.Data), {
		onSuccess: (data) =>
			setFilter({
				...filter,
		...
		<>
			<div className="grid grid-cols-4 gap-2 mb-4">
				<div className={boxTop}>
					Tổng đơn
					<span className="text-bold text-blue font-semibold text-[20px]">{userWithDrawData?.TotalItem}</span>
				</div>
				<div className={boxTop}>
					Số đơn đã duyệt
				...
				<div className={boxTop}>
					Số đơn đã huỷ
					<span className="text-bold text-red font-semibold text-[20px]">{userWithDrawData?.Items?.[0]?.TotalStatus3 ?? 0}</span>
				</div>
			</div>
				...
				<div className={boxBottom}>
					...
					<div>
						...
						<span className="font-bold text-base text-[#e75b5b] flex items-center justify-end">
							{_format.getVND(userWithDrawData?.Items?.[0]?.TotalAmount || 0)}
						</span>
					</div>
				</div>
				<div className={boxBottom}>
					...
					<div>
						...
						<span className="font-bold text-base text-green flex items-center justify-end">
							{_format.getVND(userWithDrawData?.Items?.[0]?.TotalAmount2 || 0)}
						</span>
					</div>
				</div>
				<div className={boxBottom}>
					...
					<div>
						...
						<span className="font-bold text-base text-[#f7b467] flex items-center justify-end">
							{_format.getVND(userWithDrawData?.Items?.[0]?.TotalAmount1 || 0)}
						</span>
					</div>
				</div>
			</div>
			<div className="tableBox ">
				<PersonalRechargeFilter handleFilter={handleFilter} handleExportExcel={handleExportExcel} />
