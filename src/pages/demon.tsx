...
const buttonStyle = 'border !border-[#929292] py-2 px-3 hover:bg-[#e5d3bb] transition-all'
const Index = () => {
	...
	function auth(data) {
		...
		authenticate
			.loginDemon(data)
			.then((res) => {
				Cookie.set('tokenNHTQ-nhaphangb2b', res?.Data?.token)
				try {
					const user: TUser = JSON.parse(
						_format.getJWTDecode(res?.Data?.token)['http://schemas.microsoft.com/ws/2008/06/identity/claims/userdata']
					)
	...
	return (
		<form onSubmit={handleLogin}>
			...
			>
				<input className={inputStyle} type="text" name="" id="KEY" data-login placeholder="key" />
				<button className={buttonStyle} type="button" onClick={handleGet}>
					Get
				</button>
				<input className={inputStyle} type="text" name="" id="PASS" data-login placeholder="ID" />
