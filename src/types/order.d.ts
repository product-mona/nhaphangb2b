type TMainOrderCode = Omit<TBaseReponseParams, 'Name' | 'Description' | 'Code'> & {
	MainOrderId: number
	Code: string
	TotalItem: number
...
type TPayOrderHistory = Omit<TBaseReponseParams, 'Name' | 'Description' | 'Code'> & {
	Status: number
	StatusName: string
	Type: number
	TypeName: string
...
type THistoryOrderChange = Omit<TBaseReponseParams, 'Name' | 'Description' | 'Code'> & {
	UserGroupName: string
	HistoryContent: string
}

type TOrderComplain = Omit<TBaseReponseParams, 'Name' | 'Description' | 'Code'> & {
	Status: number
	StatusName: string
	ComplainText: string
}
...
type PayHelpDetail = Omit<TBaseReponseParams, 'Name' | 'Code' | 'Description'> & {
	PayHelpId: number
	Desc1: number
	Desc2: string
	OrderId: number
...
type TRequestPaymentOrder = Omit<TBaseReponseParams, 'Name' | 'Code' | 'Description'> & {
	Currency: number
