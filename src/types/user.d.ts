...
type TUserDeposit = Omit<TBaseReponseParams, 'Code' | 'Name' | 'Description'> & {
...
type TTransportationOrder = Omit<TBaseReponseParams, 'Name' | 'Code' | 'Description'> & {
	AdditionFeeCNY: number
	AdditionFeeVND: number
	BarCode: string
	CancelReason: string
...
type TUserStatisticalDeposit = Omit<TBaseReponseParams, 'Code' | 'Name' | 'Description'> & {
	MainOrderId: number
	Note: string
	OutStockDate: Date
...
type TUserCartOrderShopTemp = Omit<TBaseReponseParams, 'Name' | 'Code' | 'Description'> & {
	Address: string
	Email: string
	FeeBuyPro: number
	FullName: string
...
type TUserHistoryTransactionVND = Omit<TBaseReponseParams, 'Name' | 'Description' | 'Code'> & {
	Amount: number
	Content: string
	MainOrderId: number
	MoneyLeft: number
...
type TUserHistoryTransactionRMB = Omit<TUserHistoryTransactionVND, 'Content'> & {
	Content: string
}

type TUserHistoryRechargeVND = Omit<TBaseReponseParams, 'Name' | 'Description' | 'Code'> & {
	UID: number
	UserName: string
	BankId: number
	Wallet: number
...
type TRechargeRMB = Omit<TBaseReponseParams, 'Code' | 'Name' | 'Description'> & {
	AcceptBy: string
	AcceptDate: Date
	Amount: number
	BankAddress: string
