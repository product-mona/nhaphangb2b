...
import { ActionButton, DataTable, FormInput, FormInputNumber, FormUpload, showToast } from '~/components'
import { TControl } from '~/types/field'
import { TColumnsType, TTable } from '~/types/table'

export const CreateOrderTable: React.FC<TControl<TUserCreateOrder> & TTable<TUserCreateOrderProduct>> = ({ control, data, remove }) => {
	const columns: TColumnsType<TUserCreateOrderProduct> = [
		...
		},
		{
			...
			align: 'center',
			render: (_, __, index) => <FormUpload image control={control} name={`Products.${index}.ImageProduct` as const} />
		},
		{
		...
		},
		{
			...
			title: 'Ghi chú',
			render: (_, __, index) => <FormInput control={control} name={`Products.${index}.NoteProduct` as const} placeholder="" />,
			responsive: ['xl']
		},
		{
			dataIndex: 'action',
		...
		expandedRowRender: (_, __, index) => (
			<ul className="px-2 text-xs">
				...
				<li className="xl:hidden justify-between flex py-2">
					...
					<FormInput control={control} name={`Products.${index}.NoteProduct` as const} placeholder="" />
				</li>
				<li className="xl:hidden justify-between flex py-2">
