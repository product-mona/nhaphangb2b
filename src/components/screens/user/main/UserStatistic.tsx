...
const box = 'grid grid-cols-2 gap-4'

const BoxItem = ({ value, path, label, icon, color }) => {
	// const boxItem =
	//   "mb-4 lg:mb-2 xl:mb-0 md:p-[10px] xl:p-4 w-[49%] lg:w-[24%] bg-[#fff] lg:my-0 shadow-xl hover:shadow-none transition-all !w-full";
	const boxItem = 'span-col-1 rounded-[10px] transition-all  bg-[#fff] xl:p-4'
	const titleContain = ''
					...
					<div className="flex justify-between mb-3 items-center">
						{value === null ? (
							...
							<div className={addOrder} style={{ color }}>
								...
								<span className="text-[10px] text-[#7a7a7a] ml-2 pb-[10px] hidden xl:block">Đơn hàng</span>
							</div>
						)}
						<div
							className={iconTitle}
		...
		<div className={box}>
			{total?.map((item) => (
				<React.Fragment key={item.key}>
					<BoxItem value={item.value} path={item.path} label={item.label} icon={item.icon} color={item.color} />
				</React.Fragment>
			))}
		</div>
	)
