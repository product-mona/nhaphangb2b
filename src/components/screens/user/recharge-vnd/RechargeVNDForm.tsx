...
import { BankCard, FormInput, FormInputNumber, FormTextarea, TableTextAreaField } from '~/components'
...
export const RechargeVNDForm: React.FC<TProps> = ({ bankCatalogue, newUser }) => {
	const [loading, setLoading] = useState(false)
	const [selectedBank, setSelectedBank] = useState(bankCatalogue[0])

	const { control, handleSubmit, reset, resetField } = useForm<TUserHistoryRechargeVND>({
		mode: 'onBlur',
		defaultValues: {
	...
	return (
		<div className="grid grid-cols-2 gap-4">
			<div className="my-4 col-span-1 bg-white rounded-sm fillterBox">
				<h3 className="text-xl font-bold uppercase text-[#595857] py-2 border-b border-[#f8dfd5]">THÔNG TIN NGÂN HÀNG</h3>
				<div
					className={`grid grid-cols-1 gap-4 max-h-[500px] overflow-y-scroll p-4 ${
						bankCatalogue.length > 4 && 'overflow-y-auto px-[5px] '
					} `}
				>
					{bankCatalogue.map((item) => {
						return <BankCard item={item} key={item?.Id} setSelectedBank={setSelectedBank} selectedBank={selectedBank} />
					})}
				</div>
			</div>
					...
					<div className="col-span-2">
						<div className="flex mb-4 items-center">
							<div className="w-full">
								<p className="mb-[5px] text-[12px] bg-white py-[2px] uppercase font-bold">Thông tin ngân hàng đã chọn!</p>
								<TableTextAreaField
									autoSize
									control={control}
									name="BankInfo"
									placeholder={
										selectedBank ? `${selectedBank?.BankInfo} - ${selectedBank?.Id}` : 'Vui lòng chọn ngân hàng!'
									}
									required={false}
									disabled
								/>
