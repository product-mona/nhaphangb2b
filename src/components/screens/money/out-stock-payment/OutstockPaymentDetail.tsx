...
export const OutstockPaymentDetail: React.FC<TTable<TMoneyOutstockPaymentDetail> & TProps> = ({
	...
	const columns: TColumnsType<TOutStockSessionPackages> = [
		...
		},
		{
			...
			title: 'Số khối (m3)',
			align: 'right',
			render: (smallPackage: TSmallPackage) => _format.getVND(smallPackage?.VolumePayment, '')
		},
		{
			dataIndex: 'SmallPackage',
			title: 'Trạng thái kiện',
			render: (smallPackage: TSmallPackage) => (
				<Tag color={smallPackageStatusData.find((x) => smallPackage?.Status === x.id)?.color}>{smallPackage?.StatusName}</Tag>
			)
		},
		{
			dataIndex: 'IsPayment',
			...
			<React.Fragment>
				<Table.Summary.Row>
					...
					<Table.Summary.Cell index={1} align="right">
						{_format.getVND(
							item?.OutStockSessionPackages?.reduce((prev, cur) => prev + cur?.SmallPackage?.PayableWeight, 0),
							' KG'
						) || '0 KG'}
					</Table.Summary.Cell>
				</Table.Summary.Row>
				<Table.Summary.Row>
					...
					<Table.Summary.Cell index={1} colSpan={1} align="right">
						{_format.getVND(
							item?.OutStockSessionPackages?.reduce((prev, cur) => prev + cur?.SmallPackage?.VolumePayment, 0),
							' m3'
						) || '0 KG'}
					</Table.Summary.Cell>
				</Table.Summary.Row>
	...
	const mutationPayment = useMutation(outStockSession.updateStatus, {
		onSuccess: (_, variables) =>
			toast.success(variables.IsPaymentWallet ? 'Thanh toán bằng ví thành công' : 'Thanh toán bằng tiền mặt thành công'),
		onError: toast.error
	})
				...
				<table className="table-preview">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								{_format.getVND(Number(item?.OutStockSessionPackages.reduce((prev, cur) => prev + cur?.TotalPriceVND, 0)))}
							</td>
						</tr>
					</tbody>
				</table>
			...
			<div className="tableBox">
				<div className="flex justify-between items-end pb-4 mb-4 border-b border-[#d7d7d7]">
					...
					<div className="flex items-center">
						...
						<Link href={type === 'payment' ? '/manager/money/out-stock-payment' : '/manager/statistical/print-purchase'}>
							<a>
