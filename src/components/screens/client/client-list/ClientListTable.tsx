...
export const ClientListTable: React.FC<TTable<TClient> & TProps> = ({
	...
	const columns: TColumnsType<TClient> = [
		...
		},
		{
			...
			title: 'Nhân viên',
			render: (_, record) => {
				const orderEm = dathangList?.Data?.Items.filter((item) => item.Id === record?.DatHangId)[0]
				const salerEm = saleList?.Data?.Items.filter((item) => item.Id === record?.SaleId)[0]
				return (
		...
		},
		{
			...
			render: (_, record) => {
				return (
					<div className="flex flex-col">
						<span className="font-bold">{warehouseTQ?.find((x) => x.Id === Number(record?.WarehouseFrom))?.Name}</span>
						<span className="font-bold">{warehouseVN?.find((x) => x.Id === Number(record?.WarehouseTo))?.Name}</span>
						<span className="font-bold">
							{shippingTypeToWarehouse?.find((x) => x.Id === Number(record?.ShippingType))?.Name}
						</span>
					</div>
				)
			},
		...
		expandedRowRender: (record) => (
			<ul className="px-2 text-xs">
				<li className="flex justify-between py-2">
					...
					{saleList?.Data?.Items.filter((item) => item.Id === record?.SaleId)[0]?.UserName}
				</li>
				<li className="flex justify-between py-2">
					<span className="font-medium mr-4">Nhân viên đặt hàng:</span>
					{dathangList?.Data?.Items.filter((item) => item.Id === record?.DatHangId)[0]?.UserName}
				</li>
				<li className="flex justify-between py-2">
