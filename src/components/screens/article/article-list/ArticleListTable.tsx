...
export const ArticleListTable: FC<TTable<TPage> & TProps> = ({ data, loading, filter, handleFilter }) => {
	...
	const columns: TColumnsType<TPage> = [
		...
		},
		{
			dataIndex: 'Active',
			title: 'Trạng thái',
			render: (_, record) => <Tag color={!record.Active ? 'red' : 'green'}>{!record.Active ? 'Ẩn' : 'Hiện'}</Tag>
		},
		{
			dataIndex: 'SideBar',
			title: 'Sibebar',
			render: (_, record) => <Tag color={!record.SideBar ? 'red' : 'green'}>{!record.SideBar ? 'Ẩn' : 'Hiện'}</Tag>
		},
		{
