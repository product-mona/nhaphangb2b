...
const BoxItem = ({ value, path, label, icon, color }) => {
	const boxItem = 'md:px-4 md:py-4 p-4 w-[100%] sm:my-3 bg-[#fff] lg:my-0 shadow-xl hover:shadow-none transition-all span-col-1'
	const titleContain = ''
...
export const OrdersPerWeek = () => {
	...
	const { data: dataChart, isLoading: isLoadingDataChart } = useQuery('get-item-in-week', () => dashboard.getItemInWeek(), {
		...
		<div className="grid grid-cols-2">
			<div className={`${box} mb-[20px] col-span-2 grid-cols-2 gap-5`}>
				{boxData.map((item, index) => (
					<BoxItem key={item.path} value={item.value} path={item.path} label={item.label} icon={item.icon} color={item.bgColor} />
				))}
			</div>
				...
				<div className="col-span-8">
					<div className="tableBox h-fit">
						<p className="xl:text-sm text-xs solid font-bold w-fit text-[#3E3C6A] pb-6 ">
							Số lượng đơn trong tháng {new Date().getMonth() + 1}
						</p>
