...
> = ({ data, handlePagination, pagination, loading, userOrder, userSale, RoleID }) => {
	// update
	const queryClient = useQueryClient()
	const mutationUpdate = useMutation((data: { order: TOrder; userId: number; type: 1 | 2 }) =>
			...
			render: (_, record) => (
				<div>
					<div className="flex items-end justify-between">
						...
						<p>{record?.CurrentCNYVN && _format.getVND(record?.CurrentCNYVN, ' Đ')}</p>
					</div>
					<div className="flex items-end justify-between">
					...
					<div className="flex items-end justify-between">
						...
						<p>{record?.TotalPriceVND && _format.getVND(record?.TotalPriceVND, ' Đ')}</p>
					</div>
					<div className="flex items-end justify-between font-bold text-[#008000]">
						<p className="mr-1">Tiền phải cọc:</p>
						<p>{record?.AmountDeposit && _format.getVND(record?.AmountDeposit, ' Đ')}</p>
					</div>
					...
					<div className="flex items-end justify-between font-bold text-[#F44336]">
						...
						<p>{record?.RemainingAmount && _format.getVND(record?.RemainingAmount, ' Đ')}</p>
					</div>
				</div>
			)
		},
					...
					<div className="text-left">
						{record.Created && (
							...
							>
								<span>Lên đơn:</span>
								<span>
									{_format.getVNDate(record.Created, 'HH:mm')} -{_format.getVNDate(record.Created, 'DD/MM/YYYY')}
								</span>
							</p>
						)}
						{record.DepositDate && (
							...
							>
								<span>Đặt cọc:</span>
								<span>
									{_format.getVNDate(record.DepositDate, 'HH:mm')} -{_format.getVNDate(record.DepositDate, 'DD/MM/YYYY')}
								</span>
							</p>
						)}
						{record.DateBuy && (
							<p
								className={clsx(
									!record.DateTQ && !record.DateVN && !record.PayDate && !record.CompleteDate && ' text-warning ',
									'flex justify-between px-2'
								)}
							>
								<span>Đặt hàng:</span>
								<span>
									{_format.getVNDate(record.DateBuy, 'HH:mm')} -{_format.getVNDate(record.DateBuy, 'DD/MM/YYYY')}
								</span>
							</p>
						)}
						{record.DateTQ && (
							<p
								className={clsx(
									!record.DateVN && !record.PayDate && !record.CompleteDate && ' text-warning ',
									'flex justify-between px-2'
								)}
							>
								<span>Đã về kho TQ:</span>
								<span>
									{_format.getVNDate(record.DateTQ, 'HH:mm')} -{_format.getVNDate(record.DateTQ, 'DD/MM/YYYY')}
								</span>
							</p>
						)}
						{record.DateVN && (
							<p className={clsx(!record.PayDate && !record.CompleteDate && 'text-warning', 'flex justify-between px-2 ')}>
								<span>Đã về kho VN:</span>
								<span>
									{_format.getVNDate(record.DateVN, 'HH:mm')} -{_format.getVNDate(record.DateVN, 'DD/MM/YYYY')}
								</span>
							</p>
						)}
						{record.PayDate && (
							<p className={clsx(!record.CompleteDate && 'text-warning ', 'flex justify-between px-2')}>
								<span>Thanh toán:</span>
								<span>
									{_format.getVNDate(record.PayDate, 'HH:mm')} -{_format.getVNDate(record.PayDate, 'DD/MM/YYYY')}
								</span>
							</p>
						)}
						{record.CompleteDate && (
						...
						{record.CancelDate && (
							<p className="text-warning font-bold flex justify-between px-2">
								<span>Huỷ:</span>
								<span>
									{_format.getVNDate(record.CancelDate, 'HH:mm')} -{_format.getVNDate(record.CancelDate, 'DD/MM/YYYY')}
								</span>
							</p>
						)}
					</div>
		...
		expandedRowRender: (record) => (
			<ul className="px-2 text-xs">
				<li className="justify-between flex py-2">
					...
					<Tag color={orderStatus.find((x) => x.id === record?.Status)?.color}>{record?.StatusName}</Tag>
				</li>
				<li className="justify-between flex py-2">
					...
					{!record?.IsCheckNotiPrice && record?.OrderType === 3 ? (
						...
						<React.Fragment>
							<span className="flex flex-col w-[50%]">
								{record?.MainOrderTransactionCodeDetails?.map((item: any, itemIndex) =>
									item?.OrderTransactionCode?.map((code, codeIndex) => (
										<div
											key={`${code}--${codeIndex}`}
											className={clsx('flex', {
								...
								{record.IsDoneSmallPackage && (
									<Tag color="#2196F3" className="!mt-2 !mr-0 w-fit self-center">
										Đã đủ MVĐ
									</Tag>
								)}
							</span>
