...
import { ActionButton, AddChildContentForm, AddNewContentForm, DataTable, EditContentForm, toast } from '~/components'
...
export const ContentMenuList: React.FC<TTable<any>> = ({ data }) => {
	...
	const columns: TColumnsType<any> = [
		...
		},
		{
			...
			render: (_, record) => (
				<Space>
					<ActionButton icon="fas fa-edit" onClick={() => setEdit(record?.Id)} title="Chỉnh sửa nội dung" />
					<ActionButton
					...
					<Popconfirm
						...
						cancelText="No"
					>
						<ActionButton icon="fas fa-trash-alt" title="Delete" iconContainerClassName="iconGreen" />
					</Popconfirm>
				</Space>
			)
		}
	...
	return (
		<React.Fragment>
			<div className="md:flex items-center justify-between pb-3 border-b border-[#333]">
				<p className="text-base text-[rgba(0,0,0,.7)] font-bold">DANH SÁCH MENU</p>
				<IconButton
						...
						expandable: {
							expandIcon: ({ expanded, onExpand, record }) =>
								record?.Children.length > 0 ? (
									<>
										{expanded ? (
											<i className="fas fa-folder-open" onClick={(e) => onExpand(record, e)}></i>
										) : (
											<i className="fas fa-folder-plus" onClick={(e) => onExpand(record, e)}></i>
										)}
									</>
								) : (
									<div className="">
							...
							expandedRowRender: (record) => {
								const OrderBy = record?.Children.sort((a, b) => a?.Position - b?.Position)
									...
									<div key={item?.Id} className="flex justify-between items-center">
										<div className="">
											...
											<Tooltip title="Trạng thái">
												<Tag color={item?.Active ? 'green' : 'red'} className="mr-4">
													{item?.Active ? 'Hiện' : 'Ẩn'}
												</Tag>
											</Tooltip>
											<Tooltip title="Tên menu con" className="ml-1 text-[12px] text-[#6b6f82]">
												{item?.Name}
											</Tooltip>
											{/* <Tooltip title="Trạng thái">
			...
			<AddNewContentForm visible={addNewModal} onCancel={() => setAddNewModal(false)} />
