...
export const ClientBenefitList: React.FC<TTable<TCustomerBenefit> & { refetchcustomerBenefits }> = ({ data, refetchcustomerBenefits }) => {
	const columns: TColumnsType<TCustomerBenefit> = [
		...
		},
		{
			...
			title: 'Ảnh',
			render: (_, record) => {
				return <Image src={record?.IMG ? record.IMG : '/pro-empty.jpg'} preview={false} className="!h-[40px] !w-[40px]" />
			}
		},
		{
			dataIndex: 'Name',
		...
		},
		{
			...
			render(value, record, index) {
				return <Tag color={record?.ItemType == 1 ? 'magenta' : 'geekblue'}>{record?.ItemTypeName}</Tag>
			}
		},
		{
			dataIndex: 'Created',
		...
		<React.Fragment>
			<DataTable
				{...{
					columns,
					data,
					title: 'DANH SÁCH QUYỀN LỢI KHÁCH HÀNG/CAM KẾT'
