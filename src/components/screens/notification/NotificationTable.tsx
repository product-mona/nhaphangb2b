...
export const NotificationTable: React.FC<TTable & TProps> = ({ data, loading, handleFilter, filter }) => {
	const columns: TColumnsType<any> = [
		...
		},
		{
			...
			width: 120,
			render: (_, data) => {
				return <Tag color={data.IsRead ? 'blue' : 'red'}>{data.IsRead ? 'Đã xem' : 'Chưa xem'}</Tag>
			}
		},
		{
			dataIndex: 'Created',
