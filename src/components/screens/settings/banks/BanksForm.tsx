...
import { Button, FormCard, FormInput, FormSwitch, FormUpload, Modal } from '~/components'
import { toast } from '~/components/toast'
import { TForm } from '~/types/table'
import { _format } from '~/utils'

export const BanksForm: React.FC<TForm<TBank>> = ({ onCancel, defaultValues, visible, btnAddTitle, title }) => {
	...
	const { isFetching, data } = useQuery(['bankId', defaultValues?.Id], () => bank.getByID(defaultValues?.Id).then((res) => res.Data), {
	...
	return (
		<Modal visible={visible} onCancel={onCancel}>
			<FormCard loading={isFetching || mutationUpdate?.isLoading || mutationAdd?.isLoading}>
					...
					<div className="grid grid-cols-2 gap-3">
						<div className="col-span-1">
							<FormInput
								...
								rules={{
									required: 'Không bỏ trống tên ngân hàng',
									maxLength: { value: 50, message: 'Không quá 50 kí tự' }
								}}
							/>
						</div>
						<div className="col-span-1">
				...
				<FormCard.Footer>
					<Button showLoading title={btnAddTitle} btnClass="!bg-active" onClick={handleSubmit(_onPress)} />
