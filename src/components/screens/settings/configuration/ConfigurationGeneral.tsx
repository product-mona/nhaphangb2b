...
export const ConfigurationGeneral: FC<TProps<TConfig1>> = ({ control, data }) => {
		...
		<React.Fragment>
			<div className="grid grid-cols-1 gap-4 px-6 mt-4">
				...
				<div className="col-span-1 mb-3">
					<FormInput control={control} label="Tên website" placeholder="Nhập hàng MONA" name="WebsiteName" required={false} />
				</div>
				...
				<div className="col-span-1 mb-3 flex grid grid-cols-4 gap-4">
					...
					<div className="">
						<FormInput control={control} label="Mã số thuế" placeholder="" name="TaxCode" required={false} />
					</div>
				</div>
				<div className="col-span-1 mb-3 grid grid-cols-3 gap-4">
					<div className="flex">
						<div className="flex flex-col items-baseline mr-4">
							<div>
								Giờ hoạt động: <span className="font-bold ml-2">{data?.TimeWork}</span>
							</div>
							<Tooltip title="Thay đổi thời gian làm việc!">
								<Switch className="!mr-4" onChange={() => setChangeTimeWork(!changeTimeWork)} />
							</Tooltip>
						</div>
						<div>
							{changeTimeWork && (
				...
				<div className="col-span-1 mb-3">
					<FormInput control={control} label="Địa chỉ 1" name="Address" placeholder="Địa chỉ 1" inputContainerClassName="mb-3" />
					<FormInput
						control={control}
						label="Địa chỉ 2"
						name="Address2"
					...
					/>
					<FormInput control={control} label="Địa chỉ 3" name="Address3" placeholder="Địa chỉ 3" required={false} />
					{/* <FormEditor
            control={control}
            label="Địa chỉ"
            name="Address"
