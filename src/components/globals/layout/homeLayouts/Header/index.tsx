...
import { ForgotPasswordForm, RegisterForm, SignInForm } from '~/components/screens/auth'
...
const Header = ({ dataConfig, dataMenu }) => {
	...
	const { data: dataUser } = useQuery(['clientData', userId], () => userApi.getByID(userId), {
	...
	return (
		<>
			<header className={styles.fixed}>
				<div className={clsx(styles.headerTop, `${y > 0 ? styles.headerTopHidden : ''}`)}>
					<div className="container fontFamilyRoboto">
							...
							<div className="flex items-center">
								{!user?.UserId || isLogOut === null ? (
									<div className="login-user flex items-center justify-end">
										...
										<span className={styles.headerTopLinkAuth}>&nbsp;/&nbsp;</span>
										<a className={styles.headerTopLinkAuth} onClick={() => setOpenModal('signIn')}>
											Đăng Nhập
										</a>
									</div>
								) : (
									<div className="relative group">
										...
										<div className="z-50 rounded-[12px] shadow-xl group-hover:scale-100 scale-0 transition-all duration-300 origin-[88%_-6%] before:content-[''] before:absolute before:right-[24px] before:top-[-28px] before:border-solid before:border-[14px] before:border-[transparent] before:border-b-[#464646] absolute right-[-20px] top-[40px] w-[300px] overflow-hidden">
											<div className={styles.expandInforHeader}>
												{
													<span className={`${dataUser?.Data?.LevelId} font-semibold text-lg`}>
											...
											<div className="bg-white">
												<div className="p-4 bg-[#f8f8f8]">
													<div className="flex mb-2 items-center justify-between">
														<span className="text-sm font-semibold text-[#000]">Level</span>
														{
															<span
																className={`${
													...
													<div className="h-5 bg-[#ebebeb] rounded-xl p-[2px]">
														<div
															...
															style={{
																width: `${(dataUser?.Data?.LevelId * 100) / 9}%`
															}}
														></div>
													</div>
												</div>
												...
												<Link href={'/user/cart'}>
													<a>
														<div className="hover:text-[#4A8916] text-[#000] transition-all duration-300 flex p-4 items-center justify-between">
															<span className="text-sm font-semibold">Giỏ hàng của bạn:</span>
															<span className="text-sm font-bold text-[#4A8916]">
																<i className="fas fa-caret-right"></i>
															</span>
														</div>
											...
											<div
												...
												onClick={() => {
													connection &&
														connection.invoke('leave', user.UserId.toString(), user.UserGroupId.toString())
			...
			<SignInForm setOpenModal={(target) => setOpenModal(target)} visible={openModal === 'signIn' ? true : false} />
			<RegisterForm setOpenModal={(target) => setOpenModal(target)} visible={openModal === 'register' ? true : false} />
			<ForgotPasswordForm setOpenModal={(target) => setOpenModal(target)} visible={openModal === 'forgetPassword' ? true : false} />
		</>
	)
}
